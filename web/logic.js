// TODO: fix page flash for dark mode
...
function update_date_menu_to_current_date() {
  var date = todays_date_txt.textContent.trim().split(", ")[1].split(" ");
  console.log(date);
  var month = date[0].split("").slice(0, 3).join("");
  var day = date[1];
  var cd_scrollers = card_date.children[0].children[1].children;
  cd_scrollers[0].scrollTop = 28 * months_abr[month];
  cd_scrollers[1].scrollTop = 28 * parseInt(day);
}
