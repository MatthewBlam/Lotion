...
window.api.receive("toRender", (data) => {
  ...
  if (call == "open_card") {
    ...
    event_element = values["open_card"][1];
...
function display_columns_events(columns) {
  ...
  Object.keys(columns).forEach(function (key) {
    ...
    Object.keys(columns[key].events).forEach(function (event) {
      ...
      date_opacity = 1;
      if (event.date.day == "Nl" || event.date.month == "Date") {
        date_opacity = 0;
      }
      var html_string = `
        <div class="event">
...
function close_delete_confirm(e) {
  ...
  if (e.target == dcc_delete) {
    column_title = card_active_column.children[0].children[0].children[0].textContent.trim();
    event_title = card_title.textContent.trim();
    sendMain({ "delete_event": [column_title, event_title] });
    close_card_container();
    column = get_column_by_title(column_title);
    event_element = get_event_by_title(column, event_title);
...
function filter_events(column, by) {
  ...
  if (by == "Date") {
    events.sort(function (a, b) {
      ...
      a = { month: months[a[0]], day: parseInt(a[1]) };
      b = { month: months[b[0]], day: parseInt(b[1]) };
      if (a.month < b.month) {
        return -1;
      }
      if (a.month > b.month) {
