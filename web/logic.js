...
function set_os_titlebar(platform) {
  ...
  document.body.style.boxShadow = "red 0px 4px 12px";
  remove_scrollbar();
}
    ...
    if (target !== co_rename_input && target !== co_delete && target !== co_filter && !target.classList.contains("mi")) {
      ...
      try {
        ...
        if (co_rename_input.getAttribute("data-open") == "true") {
          ...
          var new_column_title = new_column.children[0].children[0].children[0];
          var new_column_count = new_column.children[0].children[0].children[1];
          new_column_title.textContent = co_rename_input.value;
          if (co_rename_input.value == "") {
            new_column_title.textContent = og_column_title;
            display_error("Invalid column name");
          }
          var existing_names = Array.from(document.querySelectorAll(".column_title"));
          existing_names.forEach(function (column) {
            if (co_rename_input.value == column.textContent.trim()) {
              if (column.parentElement.parentElement.parentElement != new_column) {
                new_column_title.textContent = og_column_title;
                display_error("A column with that name already exists");
              }
            }
          });
          ...
          set_counts();
          resize_count();
          new_column_title.style.opacity = "1";
          new_column_count.style.opacity = "1";
          sendMain({ "register_column": new_column_title.textContent.trim() });
        }
      } finally {
      }
      if (co_rename_input.getAttribute("data-open") == "true") {
        ...
        if (co_rename_input.value == "") {
          new_title = og_column_title;
          display_error("Invalid column name");
        }
        var current_column = get_column_by_title(og_column_title);
        var existing_names = Array.from(document.querySelectorAll(".column_title"));
        existing_names.forEach(function (column) {
          if (new_title == column.textContent.trim()) {
            new_title = og_column_title;
            if (column.parentElement.parentElement.parentElement != current_column) {
              display_error("A column with that name already exists");
            }
          }
        });
...
function set_event_text_overflow(element, time, date) {
  if (time.style.opacity != 0) {
    time.style.pointerEvents = "auto";
    var date_width = 0;
    if (date.style.opacity != 0) {
      date.style.pointerEvents = "auto";
      date_width = date.offsetWidth + 6;
      element.children[0].children[1].style.maxWidth = "calc(100% - 120px)";
    } else {
      date.style.pointerEvents = "none";
      element.children[0].children[1].style.maxWidth = "calc(100% - 84px)";
    }
    time.style.right = `${date_width + 8}px`;
  } else {
    time.style.pointerEvents = "none";
    if (date.style.opacity == 0) {
      date.style.pointerEvents = "none";
      element.children[0].children[1].style.maxWidth = "calc(100% - 34px)";
    } else {
      date.style.pointerEvents = "auto";
      element.children[0].children[1].style.maxWidth = "calc(100% - 84px)";
    }
...
function display_columns_events(columns) {
  ...
  Object.keys(columns).forEach(function (key) {
    var html_string = `
    <div class="column" draggable="true" data-index="${columns[key].index}" data-filter="${columns[key].filter}">
    ...
    Object.keys(columns[key].events).forEach(function (event) {
      ...
      set_event_text_overflow(event_newest, ev_time, ev_date);
      set_event_size(event_newest);
    });
...
function save_card_property(property, value) {
  ...
  if (property == "title") {
    ...
    var card_time = card_active_event.children[0].children[3];
    var card_date = card_active_event.children[0].children[2];
    set_event_text_overflow(card_active_event, card_time, card_date);
  }
...
function set_event_size(event) {
  var description = true;
  if (event.children[1].textContent == "") {
    description = false;
  }
  var tags = true;
  if (event.children[2].children.length == 0) {
    tags = false;
  }
  if (description && tags) {
...
function close_card_container(dont_dissolve) {
  ...
  set_event_text_overflow(event_element, event_time, event_date);
  set_event_size(event_element);
}
...
function create_column(e) {
  var ref = Array.from(column_container.children);
  var index = parseInt(ref[ref.length - 1].getAttribute("data-index")) + 1;
  var html_string = `
    <div class="column" data-index="${index}" draggable="true" style="flex: 0 0 0; padding: 0px;">
      <div class="column_topbar">
        <div class="column_text ct">
          <div class="column_title" style="opacity: 0;"></div>
          <div class="column_count" style="opacity: 0;"></div>
        </div>
        <div class="column_buttons ct">
          <svg class="column_options_btn cb" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path class="cbp" d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z"></path></svg>
          <svg class="column_add_btn cb" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path class="cbp" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.95" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
  ...
  co_rename_input.select();
  og_column_title = title;
  setTimeout(function () {
...
function filter_events(column, by) {
  ...
  var event_list = column.children[1];
  var events = Array.from(event_list.children);
  if (by == "Tag") {
    var tags = {};
    events.forEach(function (event) {
      var tag_circles = Array.from(event.children[2].children);
      tag_circles.forEach(function (tag) {
        var tag = tag.getAttribute("data-name");
        if (tags[tag] == undefined) {
          tags[tag] = [event];
        } else {
          tags[tag] = tags[tag].concat([event]);
        }
      });
    });
    events = [];
    for (var key of Object.keys(tags)) {
      var events_of_type = Array.from(tags[key]);
      events_of_type.forEach(function (event) {
...
function create_event(e) {
  ...
  event_titles.forEach(function (title) {
    if (title.parentElement.parentElement.parentElement.parentElement == e.target.parentElement.parentElement.parentElement) {
  ...
  var html_string = `
    <div class="event" style="opacity: 0;" data-height="53">
      <div class="event_main">
        <div class="checkbox_container">
          <div class="checkbox_indicator"></div>
...
// need to drag, on the end of the drag calculate the center of every column and order accordingly

function set_column_drag_handlers() {
  var columns = Array.from(column_container.children);
  columns.forEach(function (column) {
    column.removeEventListener("dragend", column_drag_end);
    column.addEventListener("dragend", column_drag_end);
  });
}

function get_column_pos(column) {
  var rect = column.getBoundingClientRect();
  var width = rect.right - rect.left;
  var height = rect.bottom - rect.top;
  var pos = { x: rect.right - width / 2, y: rect.bottom - height / 2 };
  return pos;
}

function column_drag_end(e) {
  sort_column_order(e.target, { x: e.clientX, y: e.clientY });
}

function sort_column_order(active_column, pos) {
  var columns = Array.from(column_container.children);
  columns.sort(function (a, b) {
    if (a == active_column) {
      a = pos.x;
    } else {
      a = get_column_pos(a).x;
    }
    if (b == active_column) {
      b = pos.x;
    } else {
      b = get_column_pos(b).x;
    }
    return a - b;
  });
  ...
  columns.forEach(function (column) {
    ...
    sendMain({ "column_index": [column.children[0].children[0].children[0].textContent.trim(), index] });
    index += 1;
  });
}

function init_column_order() {
  columns = Array.from(column_container.children);
  columns.sort(function (a, b) {
    a = parseInt(a.getAttribute("data-index"));
  ...
  if (e.type == "focusout") {
    ...
    if (target == card_title) {
      if (target.textContent.trim() == "") {
        display_error("Invalid title name");
        card_title.textContent = og_title;
        card.title.blur();
        return;
      }
      var event_list = Array.from(card_active_column.children[1].children);
      var dup = false;
      event_list.forEach(function (event) {
        var event_title = event.children[0].children[1].textContent.trim();
        if (target.textContent.trim() == event_title) {
          dup = true;
        }
      });
      if (card_title.textContent.trim() == og_title) {
        dup = false;
      }
      if (dup) {
        display_error("An event with that name already exists");
        card_title.textContent = og_title;
        card_title.blur();
        return;
      }
    ...
    set_event_size(card_active_event);
  }
  if (e.type == "focus") {
    target.scrollTop = "0";
    target.scrollLeft = "0";
    ...
    og_title = card_title.textContent.trim();
  }
}
